...
class TilemapConverter:
    ...
    def convert(tmx_path: str):
        ...
        # Test
        TilemapConverter.__generate_mtileset(
            bg_upper2, os.path.splitext(os.path.basename(tmx_path))[0], tiled_map
        )

    @staticmethod
    def __generate_mtileset(
        tile_layer: pytmx.TiledTileLayer, tilemap_name: str, tiled_map: pytmx.TiledMap
    ):
        tileset_rects = {}
        ...
        mtilemap_root = f"build_ut/mtilemap/"
        os.makedirs(mtilemap_root, exist_ok=True)

        mtileset = mtileset.quantize(16)
        mtileset.save(os.path.join(mtilemap_root, f"{tilemap_name}.bmp"))
        mtileset.close()
